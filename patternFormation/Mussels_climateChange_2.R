...
D       =  0.05     # m2/h  The diffusion constant describing the movement of mussels
...
EndTime =  360*10*24/Phi # end time (days x hours)
NoFrames=  360/180*360        # Number of frames displayed during the entire simulation
...
plotdata <- data.frame(dM = d_M_Rec, biomass=MusselRec)
ggplot(data = plotdata, mapping = aes(x=dM, y=biomass)) +
geom_line(size = 1) + 
labs(x='Mortality rate', y='Average biomass') +
