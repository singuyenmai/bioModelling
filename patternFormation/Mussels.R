...
EndTime =  1000*24/Phi # end time (days x hours)
NoFrames=  360/180*1000        # Number of frames displayed during the entire simulation
...
# algae.palette= colorRampPalette(brewer.pal(9, "GnBu"))
# mussel.palette= colorRampPalette(brewer.pal(9, "Oranges"))
...
#   drM = e*c*A*M - d_M*k_M/(M + k_M)*M + D*d2_dxy2(M)
  drM = e*c*A*M - d_M*k_M/(M + k_M)*M + D*10*d2_dxy2(M)  
...
print(mean(Mortality))

# --- Average mussel biomass over time -----------------------------------------------------

avgBiomass <- data.frame(time=TimeRec, value=MusselRec)
p1 <- ggplot(data = avgBiomass, mapping = aes(x=time, y=value)) +
        geom_line(size=1) + 
        labs(x='Time (day)', y = 'Mean mussel biomass') +
        theme_bw(base_size=20)
p1

# --- Compare with and without spatial aggregations -----------------------------------------------------
biomass_1 <- MusselRec

biomass_2 <- MusselRec

avgBiomass <- data.frame(time=TimeRec, value1=biomass_1, value2=biomass_2)
avgBiomassLong <- gather(avgBiomass, key = "scenario", value = "biomass", value1, value2)
p2 <- ggplot(data = avgBiomassLong, mapping = aes(x=time, y=biomass, color=scenario)) +
        geom_line(size=1) + 
        labs(x='Time (day)', y = 'Mean mussel biomass', colour = "Diffusion constant") +
            scale_colour_manual(values = c("value1" = "#0c72b5", "value2" = "#41a735"), labels = c("D = 0.0005", "D = 0.005")) +
            theme_bw(base_size=20)
p2
